...
slz_clean$Speaker_Phonation <- paste(slz_clean$Speaker, slz_clean$Phonation_Abbr, sep = "_")
...
nmds_scores$Phonation <- slz_clean$Phonation[match(nmds_scores$Speaker_Phonation, slz_clean$Speaker_Phonation)]
nmds_scores$Phonation <- factor(nmds_scores$Phonation, levels = c("modal", 
                                                              "breathy", 
                                                              "checked", 
                                                              "rearticulated"))
...
# Perform MDS analysis
mds_dur <- metaMDS(slz_mds_duration, 
                   distance = "manhattan", 
                   wascores = TRUE, 
                   old.wa = TRUE, 
                   k = 4
...
nmds_scores_dur$Phonation <- factor(nmds_scores_dur$Phonation, levels = c("modal", 
                                                              "breathy", 
                                                              "checked", 
                                                              "rearticulated"))
# Create a ggplot2 scatter plot
...
nmds14_dur <- ggplot(nmds_scores_dur, aes(x = NMDS1, y = NMDS4, color = Phonation, label = Speaker_Phonation)) +
  geom_text() +
  theme_bw() +
  labs(title = "NMDS Plot with Duration added (Dimension 1 x Dimension 4)", x = "NMDS1", y = "NMDS4") +
  scale_color_manual(values = colorblind, name = "Phonation") + 
  theme(legend.position = "bottom")
nmds14_dur

nmds24_dur <- ggplot(nmds_scores_dur, aes(x = NMDS2, y = NMDS4, color = Phonation, label = Speaker_Phonation)) +
  geom_text() +
  theme_bw() +
  labs(title = "NMDS Plot with Duration added (Dimension 2 x Dimension 4)", x = "NMDS2", y = "NMDS4") +
  scale_color_manual(values = colorblind, name = "Phonation") + 
  theme(legend.position = "bottom")
nmds24_dur

nmds34_dur <- ggplot(nmds_scores_dur, aes(x = NMDS3, y = NMDS4, color = Phonation, label = Speaker_Phonation)) +
  geom_text() +
  theme_bw() +
  labs(title = "NMDS Plot with Duration added (Dimension 3 x Dimension 4)", x = "NMDS3", y = "NMDS4") +
  scale_color_manual(values = colorblind, name = "Phonation") +
  theme(legend.position = "bottom")
nmds34_dur
...
ggsave(filename = "figs/nmds14_dur.eps",
       plot = nmds14_dur,
       width = 6, height = 4, dpi = 300, units = "in")

ggsave(filename = "figs/nmds24_dur.eps",
       plot = nmds24_dur,
       width = 6, height = 4, dpi = 300, units = "in")

ggsave(filename = "figs/nmds34_dur.eps",
       plot = nmds34_dur,
       width = 6, height = 4, dpi = 300, units = "in")
...
p <- plotly::plot_ly(x = nmds_scores_dur$NMDS1, 
                     ...
                     type = "scatter3d", 
                     mode = "markers") %>%
  plotly::layout(scene = list(xaxis = list(title = "NMDS1"),
